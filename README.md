...
| **NL→SQL** | **5/5** mandated query patterns correct, 0 retries, 60.6% input-token saving from caching | Live run, `eval_harness` |
| **Tests** | **121 passing** | `pytest tests/` |
...
**Measured run — the five spec-mandated query patterns** (q01 aggregation, q03 filter + count, q05 group-by, q08 group-by + ranking, q10 derived age band):
...
| Median latency | 3.7 s per question |
| SQL similarity to reference | 0.955 (soft signal) |
| API requests | 10 (5 Sonnet for SQL, 5 Haiku for the prose answer) |

The remaining 20 cases — the refusal, injection, sentinel and multi-turn follow-up cases — were **not** run, to keep API spend minimal. `python -m src.talk_to_data.eval_harness` runs the full suite. Metrics with no cases in a run report as *not measured* rather than 0%, so a partial run cannot be misread as a failure.
...
**Measured on the live 5-case eval run** (10 API requests, reported by the API itself rather than modelled):
...
| Cache **reads** | 22,848 |
| Cache writes | 5,712 |
| Billable input | **12,457** vs **31,592** uncached-equivalent |
| **Input token saving** | **60.6%** |
| Cache hit rate | 72.3% |

That 60.6% is the real figure for a 5-question session; the modelled 73% assumes a longer session, over which the one-off cache write is amortised further.
